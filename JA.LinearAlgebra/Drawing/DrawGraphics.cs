...
namespace JA.Drawing
{
    public static class DrawGraphics
    {
        ...
        public static void DrawLine(this Graphics g, DrawStyles styles, PointF from, PointF to)
            => g.DrawLine(styles.Stroke, from, to);

        public static void DrawCircle(this Graphics g, DrawStyles styles, PointF center, float radius)
            => g.DrawEllipse(styles.Stroke, new RectangleF(center.X-radius, center.Y-radius, 2*radius, 2*radius));
        public static void DrawEllipse(this Graphics g, DrawStyles styles, PointF center, float majorRadius, float minorRadius)
            => g.DrawEllipse(styles.Stroke, new RectangleF(center.X-majorRadius, center.Y-minorRadius, 2*majorRadius, 2*minorRadius));
        public static void FillCircle(this Graphics g, DrawStyles styles, PointF center, float radius)
            => g.FillEllipse(styles.Fill, new RectangleF(center.X-radius, center.Y-radius, 2*radius, 2*radius));
        public static void FillEllipse(this Graphics g, DrawStyles styles, PointF center, float majorRadius, float minorRadius)
            => g.FillEllipse(styles.Fill, new RectangleF(center.X-majorRadius, center.Y-minorRadius, 2*majorRadius, 2*minorRadius));
        ...
        public static void DrawPoint(this Graphics g, DrawStyles styles, PointF coordinates, float size = -1)
        {
            size=size<0 ? styles.PointSize : size;
            var rect = new RectangleF(
                coordinates.X - size / 2,
                coordinates.Y - size / 2, size, size);
            g.DrawEllipse(styles.Stroke, rect);
        }
        ...
        /// <param name="coordinates">The point coordinates.</param>
        /// <param name="size">The point size in pixels.</param>
        public static void FillPoint(this Graphics g, DrawStyles styles, PointF coordinates, float size = -1)
        {
            size=size<0 ? styles.PointSize : size;
            var rect = new RectangleF(
                coordinates.X - size / 2,
                coordinates.Y - size / 2, size, size);
            g.FillEllipse(styles.Fill, rect);
        }
        /// <summary>
        ...
        public static void DrawPoints(this Graphics g, DrawStyles styles, PointF[] nodes, float size = -1)
        {
            size=size<0 ? styles.PointSize : size;
            for (var i = 0; i<nodes.Length; i++)
            {
                g.DrawPoint(styles, nodes[i], size);
            }
        }
        /// <summary>
        ...
        /// <param name="nodes">The array of locations defining the points</param>
        /// <remarks>Uses <see cref="Stroke"/> to draw the shape.</remarks>
        public static void FillPoints(this Graphics g, DrawStyles styles, PointF[] nodes, float size = -1)
        {
            size=size<0 ? styles.PointSize : size;
            for (var i = 0; i<nodes.Length; i++)
            {
                g.FillPoint(styles, nodes[i], size);
            }
        }
        /// <summary>
        /// Draws an ellipse
        ...
        public static void DrawEllipse(this Graphics g, DrawStyles styles, PointF center, float semiMajor, float semiMinor, float tiltAngleDegrees = 0)
        {
            float x = center.X, y = center.Y;
            float dx = semiMajor, dy = semiMinor;
            var save = g.Save();
            g.RotateTransform(-tiltAngleDegrees);
            g.DrawEllipse(styles.Stroke, x-dx, y-dy, 2*dx, 2*dy);
            g.Restore(save);
        }
        /// <summary>
        /// Fills an ellipse
        ...
        public static void FillEllipse(this Graphics g, DrawStyles styles, PointF center, float semiMajor, float semiMinor, float tiltAngleDegrees = 0)
        {
            float x = center.X, y = center.Y;
            float dx = semiMajor, dy = semiMinor;
            var save = g.Save();
            g.RotateTransform(-tiltAngleDegrees);
            g.FillEllipse(styles.Fill, x-dx, y-dy, 2*dx, 2*dy);
            g.Restore(save);
        }
        ...
        /// <param name="tiltAngleDegrees">The tilt angle degrees CCW from horizontal.</param>
        /// <remarks>Uses <see cref="Stroke"/> to draw lines.</remarks>
        public static void DrawEllipseArc(this Graphics g, DrawStyles styles, PointF center, float semiMajor, float semiMinor, float startAngleDegrees, float sweepAngleDegrees, float tiltAngleDegrees = 0)
        {
            float x = center.X, y = center.Y;
            float dx = semiMajor, dy = semiMinor;
            var save = g.Save();
            g.RotateTransform(-tiltAngleDegrees);
            g.DrawArc(styles.Stroke, x-dx, y-dy, 2*dx, 2*dy, 360-startAngleDegrees, -sweepAngleDegrees);
            g.Restore(save);
        }
        /// <summary>
        ...
        public static void DrawLineArrow(this Graphics g, DrawStyles styles, PointF p1, PointF p2, float arrowSize, bool bothSides = false)
        {
            styles.Stroke.AddEndArrow(arrowSize);
            if (bothSides)
            {
                styles.Stroke.AddStartArrow(arrowSize);
            }
            g.DrawLine(styles.Stroke, p1, p2);
            styles.Stroke.RemoveEndArrow();
            if (bothSides)
            {
                styles.Stroke.RemoveStartArrow();
            }
        }
        /// <summary>
        /// Draw a line with an arrow head.
        ...
        public static void DrawLineArrow(this Graphics g, DrawStyles styles, PointF pnt, float offset_x, float offset_y, float arrowSize, bool bothSides = false)
        {
            g.DrawLineArrow(styles, pnt, new PointF(pnt.X+offset_x, pnt.Y+offset_y), arrowSize, bothSides);
        }
        ...
        public static void DrawArcBetweenTwoPoints(this Graphics g, DrawStyles styles, PointF a, PointF b, float radius, bool flip = false)
        {
            if (flip)
            {
                (a, b)=(b, a);
            ...
            if (2*radius>=l)
            {
                ...
                g.DrawArc(styles.Stroke, C.X-radius, C.Y-radius, 2*radius, 2*radius,
        ...
        public static void DrawArcFromCenter(this Graphics g, DrawStyles styles, PointF center, PointF point, float sweepAngleDegrees)
        {
            // get distance components
        ...
        public static void DrawText(this Graphics g, DrawStyles styles, string text, PointF anchor, ContentAlignment alignment, float offset_x = 0, float offset_y = 0, int padding = 4)
        {
            var x = anchor.X;
            var y = anchor.Y;
            var size = g.MeasureString(text, styles.Font);
            switch (alignment)
            {
            ...
            g.DrawString(text, styles.Font, styles.Fill, x+offset_x, y-offset_y);
        }
        public static void DrawTextBetweenTwoPoints(this Graphics g, DrawStyles styles, string text, PointF start, PointF end, ContentAlignment alignment, float offset_x = 0, float offset_y = 0, int padding = 4)
        {
            if (start==end)
            {
                g.DrawText(styles, text, start, alignment, offset_x, offset_y, padding);
                return;
            }
            ...
            g.DrawText(styles, text, point, alignment, offset_x, offset_y, padding);
        }
        ...
        /// <param name="offset_x">(optional) Horizontal offset in pixes.</param>
        /// <param name="offset_y">(optional) Vertical offset in pixels.</param>
        public static void DrawCaption(this Graphics g, DrawStyles styles, PointF anchor, Size area, string text, ContentAlignment alingment, float offset_x = 0, float offset_y = 0)
        {
            var size = g.MeasureString(text, styles.Font);
            var p = PointF.Empty;
            ...
            g.DrawString(text, styles.Font, styles.Fill, new RectangleF(p, area), new StringFormat(f));
        }
        ...
        /// <param name="bothSides">True to draw arrows on both ends</param>
        public static void HorizontalArrow(this Graphics g, DrawStyles styles, PointF start, float length, string text, bool bothSides = false)
        {
            var sz = new SizeF(8, 8);

            if (!string.IsNullOrEmpty(text))
            {
                sz=g.MeasureString(text, styles.Font);
            }
            float x1 = start.X, x2 = x1 + length, y = start.Y;
            g.DrawLineArrow(styles, new PointF(x1, y), new PointF(x2, y), 0.4f*sz.Height, bothSides);
        ...
        /// <param name="font">The text font</param>
        /// <param name="bothSides">True to draw arrows on both ends</param>
        public static void VerticalArrow(this Graphics g, DrawStyles styles, PointF start, float length, string text, bool bothSides = false)
        {
            var sz = new SizeF(8, 8);
            ...
            g.DrawLineArrow(styles, new PointF(x, y1), new PointF(x, y2), 0.4f*sz.Height, bothSides);
